...
pub(super) async fn ensure_sftp_session(managed: &mut ManagedSession) -> Result<&mut SftpSession> {
...
pub(super) fn drain_remote_output(output: &Arc<Mutex<Vec<u8>>>) -> String {
    if let Ok(mut guard) = output.lock() {
        if guard.is_empty() {
            return String::new();
        }
...
pub(super) async fn write_to_remote_channel(
...
pub(super) async fn run_remote_copy_command(
    handle: &client::Handle<SshClientHandler>,
    source: &str,
    target: &str,
    source_is_dir: bool,
